...
public abstract class DyeableGeoArmorRenderer<T extends class_1792 & GeoItem> extends GeoArmorRenderer<T> {
    ...
    @Override
    public void renderCubesOfBone(class_4587 poseStack, GeoBone bone, class_4588 buffer, int packedLight, int packedOverlay, int colour) {
        if (this.dyeableBones.contains(bone)) {
            final Color color = getColorForBone(bone);

            colour = class_5253.class_5254.method_27763(colour, color.argbInt());
        }
