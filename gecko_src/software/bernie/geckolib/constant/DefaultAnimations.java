...
public final class DefaultAnimations {
	 ...
	 */
	public static <T extends class_1309 & GeoAnimatable> AnimationController<T> genericDeathController(T animatable) {
		return new AnimationController<>(animatable, "Death", 0, state -> state.getAnimatable().method_29504() ? state.setAndContinue(DIE) : PlayState.STOP);
	}
	 ...
	 */
	public static <T extends class_1309 & GeoAnimatable> AnimationController<T> genericAttackAnimation(T animatable, RawAnimation attackAnimation) {
	 ...
	 */
	public static <T extends class_1297 & GeoAnimatable> AnimationController<T> genericWalkRunIdleController(T entity) {
		return new AnimationController<T>(entity, "Walk/Run/Idle", 0, state -> {
			if (state.isMoving()) {
				return state.setAndContinue(entity.method_5624() ? RUN : WALK);
			}
			else {
