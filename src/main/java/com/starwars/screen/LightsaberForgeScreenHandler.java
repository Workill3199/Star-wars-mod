...
public class LightsaberForgeScreenHandler extends ScreenHandler {
    ...
    public LightsaberForgeScreenHandler(int syncId, PlayerInventory playerInventory, Inventory inventory) {
        super(ModScreenHandlers.LIGHTSABER_FORGE_SCREEN_HANDLER, syncId);
        checkSize(inventory, 4);
        this.inventory = inventory;
        inventory.onOpen(playerInventory.player);

        this.addSlot(new Slot(inventory, 0, 56, 17)); // Hilt
        this.addSlot(new Slot(inventory, 1, 56, 53)); // Emitter
        this.addSlot(new Slot(inventory, 2, 80, 35)); // Crystal
