...
public class StormtrooperEntity extends HostileEntity implements GeoEntity, RangedAttackMob {
    private final AnimatableInstanceCache cache = GeckoLibUtil.createInstanceCache(this);

    public StormtrooperEntity(EntityType<? extends HostileEntity> entityType, World world) {
    ...
    protected void initGoals() {
        ...
        this.goalSelector.add(2, new ProjectileAttackGoal(this, 1.0D, 30, 15.0F)); // Faster fire rate than Battle Droid
    ...
    public void registerControllers(AnimatableManager.ControllerRegistrar controllerRegistrar) {
        controllerRegistrar.add(new AnimationController<>(this, "controller", 0, event -> {
            if (event.isMoving()) {
                return event.setAndContinue(RawAnimation.begin().thenLoop("animation.stormtrooper.walk"));
            }
            return event.setAndContinue(RawAnimation.begin().thenLoop("animation.stormtrooper.idle"));
        }));
    }
